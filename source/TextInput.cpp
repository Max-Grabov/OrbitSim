...
void TextInput::type(const TextRenderer &text_renderer, SDL_Renderer *renderer, char input)
{
  ...
  current_text_.push_back(input);

  // So dog
  text_renderer.render(renderer, static_cast<char *>(&input), x_pixel_to_type,
                       border_.y + BORDER_OFFSET);
}
